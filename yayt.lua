...
local function startAutoTeleport()
    ...
    autoTeleportTask = task.spawn(function()
        while isAutoTeleporting do
            ...
            respawnCharacter()
            repeat task.wait(0.5) until player.Character and player.Character:FindFirstChild("HumanoidRootPart") and isAtSpawn(player.Character.HumanoidRootPart.Position, 10)
            Rayfield:Notify({Title="Loop Ulang", Content="Mulai lagi dari CP1...", Duration=2})
            task.wait(1.5)
        end
    end)
end
...
TeleTab:CreateToggle({
    Name = "⚡ Auto Teleport",
    CurrentValue = false,
    Flag = "AutoTP", -- otomatis tersimpan Rayfield
...
TeleTab:CreateSlider({
    ...
    Flag = "ATDelay", -- otomatis tersimpan Rayfield
    Callback = function(Value)
        AutoTeleportDelay = Value
    end
})

-- ===== Auto-start jika flag tersimpan =====
if Window.Flags["AutoTP"] then
    startAutoTeleport()
end
