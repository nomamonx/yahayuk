...
local SummitStatusLabel = CreateLabel(TabMisc, "🏔️ Summit: 0 | Total Global: 0 | Last: -")

local function UpdateSummitLabel()
    SummitLabel:Set(
        "Summit: " .. (summitStats.count or 0) ..
...
local ResetButton = TabMisc:CreateButton({
    ...
    Callback = function()
        ...
        -- notifikasi
        Rayfield:Notify({
            Title = "✅ File Lokal Direset",
            Content = "Global summit dan file Config.json lokal telah dihapus.",
            Duration = 4
        })

        print("[Info] File lokal berhasil dihapus dan stats di-reset.")
    end
})
