...
-- Fungsi aman set Humanoid (menunggu sampai ada)
local function SafeSetHumanoidProperty(prop, value)
...
local WalkSpeedSlider = SettingsTab:CreateSlider({
    ...
    Callback = function(Value)
        SafeSetHumanoidProperty("WalkSpeed", Value)
    end,
})
...
local JumpPowerSlider = SettingsTab:CreateSlider({
    ...
    Flag = "JumpSlider",
    Callback = function(Value)
        SafeSetHumanoidProperty("JumpPower", Value)
    end,
})

-- Infinite Jump
local UserInputService = game:GetService("UserInputService")
local InfJumpEnabled = defaultInfJump
...
-- Persist saat respawn
LocalPlayer.CharacterAdded:Connect(function(char)
    task.wait(0.5)
    SafeSetHumanoidProperty("WalkSpeed", WalkSpeedSlider.CurrentValue)
    SafeSetHumanoidProperty("JumpPower", JumpPowerSlider.CurrentValue)
    if InfJumpEnabled then EnableInfJump() end
end)

-- Reset Pengaturan (button)
SettingsTab:CreateButton({
    Name = "Reset Pengaturan",
    Callback = function()
        -- Reset slider dan properti
        WalkSpeedSlider:UpdateValue(defaultWalkSpeed)
        JumpPowerSlider:UpdateValue(defaultJumpPower)
        SafeSetHumanoidProperty("WalkSpeed", defaultWalkSpeed)
        SafeSetHumanoidProperty("JumpPower", defaultJumpPower)

        -- Reset Infinite Jump
        InfJumpToggle:UpdateValue(defaultInfJump)
        InfJumpEnabled = defaultInfJump
        DisableInfJump()
    end,
})
...
SettingsTab:CreateSection("Fitur WallHack")
