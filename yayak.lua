...
-- Tab Lainnya
-- ========================
local LainnyaTab = Window:CreateTab("Lainnya", 4483362458) -- icon bebas

local LocalPlayer = game:GetService("Players").LocalPlayer

-- ========================
-- Size Toggle (Mini / Giant)
-- ========================
LainnyaTab:CreateToggle({
    Name = "Mini / Giant Mode",
    CurrentValue = false, -- false = Mini, true = Giant
    Flag = "SizeToggle",
    Callback = function(Value)
        local char = LocalPlayer.Character
        if char then
...
local danceAnimations = {
    ["Floss"] = "rbxassetid://507771019",
    ["Dab"] = "rbxassetid://566365886",
    ["Chicken"] = "rbxassetid://148840371",
    ["Wave"] = "rbxassetid://251019695",
    ["Tai"] = "rbxassetid://6825712435" -- contoh emote tambahan
}

LainnyaTab:CreateDropdown({
    Name = "Pilih Emote / Dance",
    Options = {"Floss", "Dab", "Chicken", "Wave", "Tai"},
    CurrentOption = {},
    Flag = "DanceDropdown",
    Callback = function(Option)
        local choice = Option[1]
        local animId = danceAnimations[choice]
        local char = LocalPlayer.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
