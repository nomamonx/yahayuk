...
local DetectionTab = Window:CreateTab("Deteksi", 4483362458) -- icon bebas

-- Fungsi deteksi Admin (BillboardGui)
...
-- Fungsi deteksi Summit tertinggi (mulai dari 0)
local function DetectHighestSummit()
    local highestSummit = 0
            ...
            AdminLoop = task.spawn(function()
                ...
                while true do
                    ...
                    if #admins > 0 then
                        ...
                        AdminLabel:Set("Admin Terdeteksi: 0")
                        lastAdmins = {}
                    end
                    task.wait(5)
                end
...
-- Toggle Summit Detection
local SummitLoop
DetectionTab:CreateToggle({
    Name = "Check Summit Tertinggi",
    CurrentValue = false,
    Flag = "CheckSummitToggle",
    Callback = function(Value)
        if Value then
            SummitLoop = task.spawn(function()
                local lastPlayer, lastSummit = nil, -1
                while true do
                    local highestPlayer, highestSummit = DetectHighestSummit()
                    if highestPlayer then
                        SummitLabel:Set("Summit Tertinggi: " .. highestPlayer .. " (" .. highestSummit .. ")")
                        if highestPlayer ~= lastPlayer or highestSummit ~= lastSummit then
                            Rayfield:Notify({
                                Title = "Summit Baru Terdeteksi!",
                                Content = highestPlayer .. " sekarang Summit: " .. highestSummit,
                                Duration = 5,
                            })
                            lastPlayer, lastSummit = highestPlayer, highestSummit
                        end
                    else
                        SummitLabel:Set("Summit Tertinggi: -")
                        lastPlayer, lastSummit = nil, -1
                    end
                    task.wait(5)
                end
            end)
...
-- Label info (DIPINDAH KE BAWAH)
AdminLabel = DetectionTab:CreateLabel("Admin Terdeteksi: 0")
SummitLabel = DetectionTab:CreateLabel("Summit Tertinggi: -")
