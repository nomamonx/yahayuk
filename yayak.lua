...
local DetectionTab = Window:CreateTab("Detection", 4483362458) -- icon ID bebas

-- ========================
-- Helper Functions
-- ========================

-- Ambil summit dari player (BillboardGui / leaderstats)
local function GetPlayerSummit(player)
    local summitNumber = nil

    -- 1) Cek BillboardGui di kepala
    ...
    -- 2) Kalau BillboardGui tidak ada → cek leaderstats
...
-- Deteksi semua admin + summit mereka
...
-- Deteksi top N summit tertinggi
...
-- ========================
-- Admin Detection Toggle
-- ========================
local AdminLoop
local AdminLabel = DetectionTab:CreateLabel("Admin Terdeteksi: 0")
        ...
        if Value then
            AdminLoop = task.spawn(function()
                ...
                while true do
                    ...
                    if #admins > 0 then
                        local labelText = "Admin Terdeteksi (" .. tostring(#admins) .. "):"
                        local notifyText = ""
                        ...
                        AdminLabel:Set(labelText)

                        if notifyText ~= lastAdmins then
                            Rayfield:Notify({
                                Title = "Admin Terdeteksi!",
                                Content = "Total: " .. tostring(#admins) .. notifyText,
                                Duration = 7,
                            })
...
-- ========================
-- Summit Detection Toggle
-- ========================
local SummitLoop
local SummitLabel = DetectionTab:CreateLabel("Top Summit: -")

DetectionTab:CreateToggle({
    Name = "Check Summit Top 3",
    CurrentValue = false,
