...
local function OnSummitReachedEnhanced()
    ...
    -- webhook dengan jumlah player & nama disensor
    pcall(function()
        ...
        SendWebhook("✅🏔️ Summit Reached 🎉", content, { color = 3066993 })
    end)

    -- tambahan: notif join server (sekali aja)
    if not summitStats.joinNotified then
        summitStats.joinNotified = true
        pcall(function()
            local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
            local joinLink = string.format("roblox://placeId=%d&gameInstanceId=%s", game.PlaceId, tostring(JobId))
            local embed = {
                title = "🎉 Player Join Server",
                description = string.format("👤 **%s** masuk ke server!\n🔗 [Klik Join Server](%s)", playerName, joinLink),
                color = 3447003,
                timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
            }
            local body = HttpService:JSONEncode({ embeds = { embed } })
            SendWebhookRaw(Config.WebhookURL, body)
        end)
    end

    -- tambahan: status player online + ping
    pcall(function()
        local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
        local stats = LocalPlayer and LocalPlayer:FindFirstChild("Network") or nil
        local ping = (stats and stats:FindFirstChild("Ping")) and stats.Ping.Value or 0

        local embed = {
            title = "📡 Status Player",
            description = string.format("👤 **%s**\n📡 Status: **ONLINE**\n📶 Ping: **%d ms**", playerName, ping),
            color = 3066993,
            timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
        }
        local body = HttpService:JSONEncode({ embeds = { embed } })
...
-- Tambahan: offline notif (player keluar server)
Players.PlayerRemoving:Connect(function(plr)
    if plr == LocalPlayer then
        local embed = {
            title = "📴 Player Offline",
            description = string.format("👤 **%s** keluar / disconnect dari server.", plr.Name),
            color = 15158332,
            timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ")
        }
        local body = HttpService:JSONEncode({ embeds = { embed } })
