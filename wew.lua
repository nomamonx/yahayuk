...
Players.PlayerRemoving:Connect(function(plr)
    if plr == LocalPlayer and not alreadyOffline then
        ...
        if isHopping then
            local desc = string.format("🔀 **%s** sedang hop server.\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s",
                playerName, ping, GetPlaytime(), GetTimestamp())
            SendWebhookWithReaction("🔀 Hop Server", desc, 0x9b59b6, "🔀")
            isHopping = false
            joinTime = tick()
        else
        ...
        if LocalPlayer and LocalPlayer.Parent == Players then
            ...
            if not alreadyOffline then
                ...
                elseif tick() - lastSeen <= 2 then
                    ...
                    local desc = string.format("❌ **%s** disconnect (Unknown).\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s",
                        playerName, ping, GetPlaytime(), GetTimestamp())
