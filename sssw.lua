...
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local oldJobId, lastSeen, alreadyOffline, joinTime = tostring(game.JobId), tick(), false, tick()

-- Fungsi untuk kirim webhook notif
local function SendNotif(title, color, emoji, msg)
    local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
    ...
    local desc = string.format("%s\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s", msg, ping, playTime, GetTimestamp())
...
game:BindToClose(function()
    ...
    alreadyOffline = true
    SendNotif("📴 Player Keluar Manual", 0xe67e22, "🚪", "📴 "..(LocalPlayer and LocalPlayer.Name or "Unknown").." keluar dari game (Exit Game)")
end)

-- === Hop Server (manual / auto) ===
if LocalPlayer then
    LocalPlayer.OnTeleport:Connect(function(state)
        if state == Enum.TeleportState.Started then
            ...
            SendNotif("🔄 Player Hop Server", 0x9b59b6, "🔄", "🔄 "..(LocalPlayer and LocalPlayer.Name or "Unknown").." pindah server!\n🌐 Dari: "..oldJobId.."\n➡️ Ke: "..newJobId.."\n🔗 [Join Server Baru]( "..joinLink.." )")
            oldJobId = newJobId
            joinTime = tick()
        end
    end)
    ...
    while task.wait(1) do
        if LocalPlayer and LocalPlayer.Parent == Players then
            ...
            if not alreadyOffline then
                ...
                if delta <= 5 then
                    SendNotif("⚠️ Player Force Close", 0xd35400, "💥", "💥 "..(LocalPlayer and LocalPlayer.Name or "Unknown").." keluar tiba-tiba (Force Close)")
                else
                    SendNotif("📴 Player Offline", 0xe74c3c, "❌", "❌ "..(LocalPlayer and LocalPlayer.Name or "Unknown").." disconnect (Unknown)")
                end
            end
            break
        end
...
task.spawn(function()
    while task.wait(300) do
        ...
        if not (LocalPlayer and LocalPlayer.Parent == Players) then
            alreadyOffline = true
            SendNotif("🚫 Player Banned / Kicked", 0xf1c40f, "⛔", "⛔ Akun "..(LocalPlayer and LocalPlayer.Name or "Unknown").." dikeluarkan paksa (Ban/Kick)")
            break
        end
    end
end)
...
function AutoHop()
    if alreadyOffline then return end
    alreadyOffline = true
    SendNotif("🔀 Auto Hop Server", 0x9b59b6, "🔀", "🔀 Auto Hop Server aktif!\n👤 "..(LocalPlayer and LocalPlayer.Name or "Unknown"))
    AutoHopServer()
end
