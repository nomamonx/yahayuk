...
local function OnSummitReachedEnhanced()
    ...
    -- Embed utama (summit info)
    local desc = string.format(
        "👤 Player: %s (%s)\n🖥️ Server: %s\n👥 Orang di server: %d\n⛰️ Summit (server): %d\n🌍 Total Summit: %d\n⏱️ Waktu: %s\n📶 Ping: %d ms",
        maskedName, spoilerName, tostring(JobId), playerCount,
        summitStats.count, GlobalStats.TotalSummits, FormatTimeSec(duration), ping
    )

    SendWebhookWithReaction("✅🏔️ Summit Reached 🎉", desc, 0x2ecc71, "⛰️")

    -- Auto hop kalau limit tercapai
    if summitStats.count >= SummitAutoHopLimit then
        AutoHopServer()
    end
end

-- === Join Server langsung saat masuk ===
task.spawn(function()
    task.wait(3) -- kasih delay biar pasti loaded
    local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
...
Players.PlayerRemoving:Connect(function(plr)
    if plr == LocalPlayer then
        local desc = string.format("👤 **%s** keluar / disconnect.", plr.Name)
        SendWebhookWithReaction("📴 Player Offline", desc, 0xe74c3c, "❌")
    end
end)

-- === Monitor 5 menit (ban/kick/online status) ===
task.spawn(function()
    while task.wait(300) do
        local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
        if LocalPlayer and LocalPlayer.Parent == Players then
            local ping = (LocalPlayer.GetNetworkPing and LocalPlayer:GetNetworkPing())
                and math.floor(LocalPlayer:GetNetworkPing() * 1000) or 0
            local desc = string.format("👤 **%s**\n📡 Status: **ONLINE**\n📶 Ping: **%d ms**", playerName, ping)
            SendWebhookWithReaction("📡 Status Player", desc, 0x2ecc71, "✅")
        else
            local desc = string.format("⚠️ Akun **%s** terdeteksi keluar secara paksa (ban/kick).", playerName)
            SendWebhookWithReaction("🚫 Player Banned / Kicked", desc, 0xf1c40f, "⚠️")
            break
        end
    end
end)
