...
local SummitLabel = CreateLabel(TabDetector, "Summit: 0 | Last: 00m:00s")
...
local function OnSummitReachedEnhanced()
    ...
    -- update UI
    pcall(function()
        SummitLabel:Set("Summit: "..summitStats.count.." | Total Global: "..GlobalStats.TotalSummits.." | Last: " .. FormatTimeSec(summitStats.lastTime))
    end)
    ...
    -- kirim webhook (seperti biasa)
    local desc = string.format(
        "Player: **%s**\nSummit di Server Ini: %d\nTotal Global: %d\nWaktu Summit Terakhir: %ds (%s)\nServer: %s\nWaktu: %s",
        tostring(LocalPlayer and LocalPlayer.Name or "Unknown"),
        summitStats.count,
        GlobalStats.TotalSummits,
    ...
    SendWebhook("🏔️ Summit Notification", desc, {
        color = 3066993,
        fields = {
            { name = "Total Summit", value = tostring(summitStats.count), inline = true },
             { name = "Total Global", value = tostring(GlobalStats.TotalSummits), inline = true }
            { name = "Durasi Terakhir", value = FormatTimeSec(summitStats.lastTime), inline = true },
            { name = "ServerId", value = tostring(JobId), inline = false }
        }
    })

    if Config.NotifyInGame then
        Notify("🏔️ Summit", "Summit ke-"..summitStats.count.." tercapai! Durasi: " .. FormatTimeSec(summitStats.lastTime))
    end

    -- auto hop kalau sudah limit
