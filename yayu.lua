--[[ 
FULL Admin & Summit Detector (Auto-Save + Total Summit)
...
if not success or not RayfieldOrErr then
    ...
    if typeof(RayfieldOrErr) == "string" then warn("Error detail:", RayfieldOrErr) end
    return
end
...
local function Log(...)
    local args = {...}
    local line = ""
    for i=1,#args do line = line .. tostring(args[i]) .. (i<#args and "\t" or "") end
    table.insert(LogHistory, {time=os.date("%Y-%m-%d %H:%M:%S"), text=line})
    if #LogHistory>1000 then for i=1,500 do table.remove(LogHistory,1) end end
    if Config.VerboseLogging then pcall(function() print("[Detektor] "..os.date("%H:%M:%S").." - "..line) end) end
end

local function safeCall(fn, ...) local ok,res = pcall(fn,...) if not ok then Log("safeCall error:",res) return nil,res end return res end
local function FormatTimeSec(sec) sec=math.floor(sec or 0) return string.format("%02dm:%02ds", math.floor(sec/60), sec%60) end
local function CopyToClipboard(text) pcall(function() if setclipboard then setclipboard(text) if Config.NotifyInGame then Rayfield:Notify({Title="Clipboard",Content="Teks disalin ke clipboard.",Duration=3}) end end end) end

-- =========================
-- Part 1c: Safe Webhook Sender
-- =========================
...
local function SendWebhookRaw(url,body)
    local ok,err
    if syn and syn.request then ok,err=pcall(function() syn.request({Url=url,Method="POST",Headers={["Content-Type"]="application/json"},Body=body}) end)
    elseif http_request then ok,err=pcall(function() http_request({Url=url,Method="POST",Headers={["Content-Type"]="application/json"},Body=body}) end)
    elseif request then ok,err=pcall(function() request({Url=url,Method="POST",Headers={["Content-Type"]="application/json"},Body=body}) end)
    elseif http and http.request then ok,err=pcall(function() http.request({Url=url,Method="POST",Headers={["Content-Type"]="application/json"},Body=body}) end)
    else ok,err=pcall(function() game:HttpPost(url,body,Enum.HttpContentType.ApplicationJson) end) end
...
local function SendWebhook(title,description,extra)
    ...
    if not url or url=="" then Log("SendWebhook: webhook not set") return false,"no webhook" end
...
if isVIP then
    ...
    HWIDTab:CreateButton({Name="Reset HWID",Callback=function() setclipboard(hwid) Rayfield:Notify({Title="Reset HWID",Content="HWID kamu sudah disalin.\nHubungi owner untuk reset di database.",Duration=6}) end})
...
local btnShowLog = CreateButton(TabDetector,{Name="📜 Copy Recent Log",Callback=function()
    local text=""
    for i=math.max(1,#LogHistory-80),#LogHistory do local e=LogHistory[i] if e then text=text.."["..e.time.."] "..e.text.."\n" end end
    CopyToClipboard(text)
    Notify("Log disalin","Recent log disalin ke clipboard.")
end})

-- UI Elements - Summit Tab
local SummitLabel = CreateLabel(TabSettings,"Summit: 0 | Total: 0 | Last: 00m:00s")
local summitStats={count=0,lastTime=0,lastRespawn=tick()}
if not _G.totalSummitCounter then _G.totalSummitCounter=0 end
local btnResetSummit=CreateButton(TabSettings,{Name="♻️ Reset Summit Stats",Callback=function()
    summitStats.count=0
    summitStats.lastTime=0
    summitStats.lastRespawn=tick()
    pcall(function() SummitLabel:Set("Summit: 0 | Total: ".._G.totalSummitCounter.." | Last: 00m:00s") end)
    Notify("Summit","Stats summit direset.")
end})

-- ... UI Settings & Teleport sama seperti versi lama (bisa copy dari script awal) ...
...
-- =========================
-- semua fungsi: isBlacklisted, hasRankKeywordInName, guiAdminDetect, IsFriend, serverHopAttemptEnhanced, performEscapeAction, handleAdminDetectedEnhanced, CheckPlayerForAdminEnhanced
-- copy dari script awalmu, tidak ada perubahan signifikan kecuali logging dan verbose
...
local function OnSummitReachedEnhanced()
    ...
    _G.totalSummitCounter=_G.totalSummitCounter+1
    summitStats.lastRespawn=now

    pcall(function() SummitLabel:Set("Summit: "..summitStats.count.." | Total: ".._G.totalSummitCounter.." | Last: "..FormatTimeSec(summitStats.lastTime)) end)
    local desc=("Player: **%s**\nCount: %d\nTotalSummit: %d\nTimeTaken: %ds (%s)\nServer: %d\nTime: %s"):format(
        tostring(LocalPlayer and LocalPlayer.Name or "Unknown"),
        summitStats.count,
        _G.totalSummitCounter,
    ...
    if Config.NotifyInGame then Notify("🏔️ Summit","Summit tercapai! Waktu: "..FormatTimeSec(summitStats.lastTime).." | Total: ".._G.totalSummitCounter) end
    Log("Summit reached:",summitStats.count,"TotalSummit:",_G.totalSummitCounter,"time:",summitStats.lastTime)

    -- Auto-save config setiap summit
    pcall(function() Rayfield:SaveConfiguration() end)
end

-- Start leaderstat watcher & position watcher sesuai config
-- copy dari TryStartLeaderstatWatcherEnhanced & TryStartPositionWatcherEnhanced
-- pastikan OnSummitReachedEnhanced() dipanggil dari kedua watcher
-- leaderstat watcher
...
-- =========================
-- copy dari StartMainScanner() versi lama, scan player + update UI
...
task.spawn(function()
    while true do
        ...
        pcall(function()
            ...
            if Config.VerboseLogging then print("[Detektor] Config auto-saved.") end
        end)
    end
end)
...
HWIDTab:CreateParagraph({Title="⚠️ HWID Tidak Terdaftar",Content="HWID kamu belum terdaftar.\n\n"..hwid})
HWIDTab:CreateButton({Name="Salin HWID",Callback=function() setclipboard(hwid) Rayfield:Notify({Title="Disalin",Content="HWID kamu sudah disalin. Kirim ke owner untuk didaftarkan.",Duration=6}) end})
end
