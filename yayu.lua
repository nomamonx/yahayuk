...
-- TextBox untuk input URL webhook langsung dari game
local inputWebhook = CreateTextBox(TabSettings,{
    Name="Webhook URL",
    PlaceholderText="Masukkan URL webhook Discord",
    Text=Config.WebhookURL or "",
    Callback=function(txt)
        Config.WebhookURL = txt or ""
        Notify("Webhook", "URL webhook disimpan: "..(txt or "kosong"))
    end
})

-- Toggle enable/disable pengiriman webhook
...
-- Toggle embed/not embed di webhook
...
-- Tombol Test Webhook
CreateButton(TabSettings,{
    Name="🔗 Test Webhook",
    Callback=function()
        if not Config.WebhookURL or Config.WebhookURL=="" then
            Notify("Webhook Error","Webhook belum diatur!")
            return
        end
        local ok, err = pcall(function()
            SendWebhook("✅ Test Webhook", "Ini adalah test notifikasi dari script detektor.")
        end)
        if ok then
            Notify("Webhook Test","Berhasil mengirim test webhook!")
        else
            Notify("Webhook Test Failed","Gagal mengirim webhook: "..tostring(err))
        end
    end
})
