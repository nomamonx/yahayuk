...
-- Auto Teleport
-- ===== Config tambahan untuk Auto Teleport =====
Config.AutoTeleportEnabled = Config.AutoTeleportEnabled or false
Config.AutoTeleportDelay = Config.AutoTeleportDelay or 2
...
local function startAutoTeleport()
    ...
    autoTeleportTask = task.spawn(function()
        while Config.AutoTeleportEnabled do
            for i, cf in ipairs(autoTeleportPoints) do
                if not Config.AutoTeleportEnabled then break end
                local char = player.Character
                if char and char:FindFirstChild("HumanoidRootPart") then
                    char.HumanoidRootPart.CFrame = cf
                    Rayfield:Notify({ Title="Auto Teleport", Content="Teleport ke CP "..i, Duration=2 })
                end
                task.wait(Config.AutoTeleportDelay)
            end
            -- Respawn karakter setelah loop
            respawnCharacter()
            task.wait(1.5)
        end
    end)
...
local function stopAutoTeleport()
    Config.AutoTeleportEnabled = false
...
TeleTab:CreateToggle({
    Name = "⚡ Auto Teleport",
    CurrentValue = Config.AutoTeleportEnabled,
...
TeleTab:CreateSlider({
    ...
    Increment = 0.5,
    Suffix = "s",
    CurrentValue = Config.AutoTeleportDelay,
    Callback = function(Value)
        Config.AutoTeleportDelay = Value
    end
})

-- ===== Jalankan Auto Teleport otomatis saat load jika enabled =====
