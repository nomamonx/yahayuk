        ...
        if JogetEnabled then
            ...
            JogetConnection = RunService.Heartbeat:Connect(function(dt)
                ...
                if char then
                    ...
                    if leftArm then
                        leftArm.CFrame = leftArm.CFrame * CFrame.Angles(0, 0, math.rad(math.sin(t) * JogetAmplitude))
                    end
                    if rightArm then
                        rightArm.CFrame = rightArm.CFrame * CFrame.Angles(0, 0, math.rad(-math.sin(t) * JogetAmplitude))
                    end
                end
            end)
        else
...
-- Toggle Kepala di bawah / terbalik
local InvertEnabled = false
FunTab:CreateToggle({
    Name = "Kepala di bawah",
    CurrentValue = false,
    Flag = "InvertToggle",
    Callback = function(value)
        InvertEnabled = value
