...
-- ========================
-- FunTab: Sit on Player
-- ========================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local LocalPlayer = Players.LocalPlayer

local FunTab = Window:CreateTab("Fun", 4483362458) -- icon bebas
...
-- Fungsi sit di atas kepala target
local function SitOnPlayer()
    if not TargetPlayer or not TargetPlayer.Character then return end
    local char = LocalPlayer.Character
    local targetHRP = TargetPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not char or not char:FindFirstChild("HumanoidRootPart") or not targetHRP then return end
    ...
    local attach0 = Instance.new("Attachment", char.HumanoidRootPart)
    ap.Attachment0 = attach0
    ap.MaxForce = 1e5
    ap.Responsiveness = 50
    ap.Parent = char.HumanoidRootPart

    local ao = Instance.new("AlignOrientation")
    local attach1 = Instance.new("Attachment", char.HumanoidRootPart)
    ao.Attachment0 = attach1
    ao.MaxTorque = 1e5
    ...
    conn = RunService.RenderStepped:Connect(function()
        if SitEnabled and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ap.Position = TargetPlayer.Character.HumanoidRootPart.Position + Vector3.new(0,3,0)
        else
            conn:Disconnect()
        end
    end)
...
local function StopSit()
    for _, obj in pairs(AlignObjects) do
