...
local SummitLoop
DetectionTab:CreateToggle({
    Name = "🏔️ Cek Summit / Top Puncak",
    CurrentValue = Config.CheckSummit or false,
    Flag = "CheckSummitToggle",
    Callback = function(Value)
        SetConfig("CheckSummit", Value)
        if Value then
                ...
                while true do
                    ...
                    for _, player in ipairs(Players:GetPlayers()) do
                        if player ~= LocalPlayer then
                            if string.find(string.lower(player.DisplayName), "summit")
                            or string.find(string.lower(player.Name), "summit") then
                                table.insert(summitList, player.DisplayName .. " ("..player.Name..")")
                            end
                        end
                    end
                    if #summitList > 0 then
                        SummitLabel:Set("Top Puncak: " .. table.concat(summitList, ", "))
                    else
                        SummitLabel:Set("Top Puncak: -")
                    end
                    task.wait(5)
                end
            end)
