...
-- duduk (lokal, posisi tetap)
local function sitHere()
    ...
    -- bikin attachment supaya tubuhmu "diam" seperti duduk
    ...
    ap.Attachment0 = a0
    ap.Position = hrp.Position -- kunci di posisi sekarang
...
-- ✅ Toggle di Rayfield (misalnya FunTab)
FunTab:CreateToggle({
    Name = "Duduk (Stay)",
    CurrentValue = false,
    Flag = "SitHere",
    Callback = function(Value)
        ...
        if sitActive then
            ...
            Rayfield:Notify({
                Title = "Duduk Aktif",
                Content = "Kamu duduk di posisi sekarang.",
                Duration = 3
            })
        else
            unSit()
            Rayfield:Notify({
                Title = "Duduk Nonaktif",
                Content = "Kamu berdiri lagi.",
                Duration = 3
            })
        end
    end,
