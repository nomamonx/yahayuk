...
SettingsTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            ...
            -- Reset Infinite Jump
            InfJumpToggle:SetValue(defaultInfJump)
            InfJumpEnabled = defaultInfJump
            DisableInfJump()

            -- Matikan toggle reset supaya bisa ditekan lagi nanti
            task.wait(0.1)
            SettingsTab.Flags.ResetSettings.Value = false
        end
    end,
})
...
local function CreateNameESP(player)
    ...
    -- Loop update jarak
    task.spawn(function()
        while ESPEnabled and ESPObjects[player] do
