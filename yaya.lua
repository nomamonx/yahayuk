...
-- =========================================
-- Nempel/Duduk di Kepala Pemain Lain (Fun Tab)
-- =========================================
-- =========================================
-- Nempel/Duduk di Kepala Pemain (Fun Tab)
-- =========================================
...
local function RefreshPlayerList()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            table.insert(names, plr.DisplayName) -- pakai DisplayName
        end
    end
    return names
end

local SelectedPlayerDropdown = FunTab:CreateDropdown({
    Name = "Pilih Player",
    Options = RefreshPlayerList(),
    CurrentOption = {},
...
FunTab:CreateButton({
    Name = "Refresh Daftar Player",
    Callback = function()
        SelectedPlayerDropdown:Refresh(RefreshPlayerList(), true)
    end,
})
    ...
    Callback = function()
        ...
        if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("Head") then
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                -- Buat WeldConstraint untuk nempel di kepala target
                local weld = Instance.new("WeldConstraint")
                weld.Part0 = char.HumanoidRootPart
                weld.Part1 = targetPlayer.Character.Head
                weld.Parent = attachmentPart

                -- Naik sedikit agar kayak duduk di kepala
                attachmentPart.CFrame = targetPlayer.Character.Head.CFrame + Vector3.new(0, 2, 0)

                Rayfield:Notify({
                    Title = "Fun",
                    Content = "Sekarang kamu menempel di kepala " .. targetPlayer.DisplayName,
                    Duration = 5,
                })
            end
        else
            Rayfield:Notify({
                Title = "Error",
                Content = "Tidak ada pemain yang dipilih atau pemain tidak memiliki karakter!",
                Duration = 5,
            })
        end
    end,
...
FunTab:CreateButton({
    ...
    Callback = function()
        if attachmentPart then
            ...
            Rayfield:Notify({
                Title = "Fun",
                Content = "Kamu sudah berhenti menempel di kepala pemain.",
                Duration = 5,
            })
        else
            Rayfield:Notify({
                Title = "Fun",
                Content = "Kamu tidak sedang menempel di kepala siapa pun.",
                Duration = 5,
            })
        end
    end,
