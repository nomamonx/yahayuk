...
-- ========================
-- Fly System (Kamera Direction, PC & Mobile Stabil)
-- ========================
local Players = game:GetService("Players")
local UserInputService = game:GetService("UserInputService")
local RunService = game:GetService("RunService")
...
local function startFly()
    ...
    flyConn = RunService.RenderStepped:Connect(function()
        ...
        -- Mobile (Analog bawaan)
        if UserInputService.TouchEnabled then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                local input = hum.MoveDirection
                if input.Magnitude > 0 then
                    -- Ambil axis X/Z lalu gabung dengan kamera
                    moveDir += camCF.RightVector * input.X
                    moveDir += camCF.LookVector * input.Z
                end
            end
        end
...
-- Tab Fly
-- ========================
local TabFly = Window:CreateTab("Fly")

TabFly:CreateToggle({
    Name = "Fly (Stabil Kamera)",
    CurrentValue = false,
    Flag = "FlyToggle",
    Callback = function(Value)
        if Value then
