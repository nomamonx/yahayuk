    ...
    Callback = function(value)
        ...
        if DanceEnabled then
            ...
            DanceConnection = RunService.Heartbeat:Connect(function()
                local char = LocalPlayer.Character
                if char then
                    local angles = math.sin(tick() * 5) * math.rad(20) -- goyang 20 derajat
                    if char:FindFirstChild("LeftUpperArm") then
                        char.LeftUpperArm.CFrame = CFrame.Angles(angles,0,0)
                    end
                    if char:FindFirstChild("RightUpperArm") then
                        char.RightUpperArm.CFrame = CFrame.Angles(-angles,0,0)
                    end
                    if char:FindFirstChild("LeftUpperLeg") then
                        char.LeftUpperLeg.CFrame = CFrame.Angles(-angles/2,0,0)
                    end
                    if char:FindFirstChild("RightUpperLeg") then
                        char.RightUpperLeg.CFrame = CFrame.Angles(angles/2,0,0)
                    end
                end
            end)
        else
            ...
            local char = LocalPlayer.Character
            if char then
                for _, part in ipairs({"LeftUpperArm","RightUpperArm","LeftUpperLeg","RightUpperLeg"}) do
                    if char:FindFirstChild(part) then
                        char[part].CFrame = CFrame.new()
                    end
                end
            end
        end
