...
-- Pastikan ada RemoteEvent di ReplicatedStorage bernama "PlayExplosion"
local remote = ReplicatedStorage:FindFirstChild("PlayExplosion")
if not remote then
    remote = Instance.new("RemoteEvent")
    remote.Name = "PlayExplosion"
    remote.Parent = ReplicatedStorage
end

-- Tombol Prank Ledakan Global
FunTab:CreateButton({
    Name = "Prank Ledakan (Global)",
    Callback = function()
        -- Kirim request ke server untuk memainkan ledakan
        remote:FireServer()
        Rayfield:Notify({
            Title = "Fun",
            Content = "Prank ledakan dikirim ke server!",
            Duration = 4,
        })
    end,
})
