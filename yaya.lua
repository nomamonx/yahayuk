...
local DistanceBack = 10  -- jarak belakang target
local Height = 5         -- tinggi kamera dari HumanoidRootPart
local Smooth = 0.2       -- lerp untuk smooth movement
...
local PlayerDropdown = SettingsTab:CreateDropdown({
    ...
    Callback = function(opt)
        for _, p in ipairs(Players:GetPlayers()) do
            if p.DisplayName == opt[1] then
                SpectateTarget = p
                -- langsung teleport kamera ke target saat dipilih
                if SpectateTarget.Character and SpectateTarget.Character:FindFirstChild("HumanoidRootPart") then
                    local root = SpectateTarget.Character.HumanoidRootPart
                    local lookVector = root.CFrame.LookVector
                    Camera.CFrame = CFrame.new(root.Position - lookVector * DistanceBack + Vector3.new(0, Height, 0), root.Position)
                end
                break
            end
        end
...
SettingsTab:CreateToggle({
    ...
    Callback = function(val)
        ...
        if SpectateEnabled then
            ...
            -- langsung teleport kamera ke target saat toggle diaktifkan
...
RunService.RenderStepped:Connect(function()
    if SpectateEnabled and SpectateTarget and SpectateTarget.Character and SpectateTarget.Character:FindFirstChild("HumanoidRootPart") then
        local root = SpectateTarget.Character.HumanoidRootPart
        local lookVector = root.CFrame.LookVector
        local desiredPos = root.Position - lookVector * DistanceBack + Vector3.new(0, Height, 0)
        Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(desiredPos, root.Position), Smooth)
    end
end)

-- Refresh saat player join
Players.PlayerAdded:Connect(function()
    PlayerDropdown:Refresh(RefreshPlayers(), true)
end)
