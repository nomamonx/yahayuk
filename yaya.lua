...
local function AutoTeleportLoop()
    ...
    while isAutoTeleporting do
       ...
       -- 2. Teleport ke Puncak
            ...
            task.wait(AutoTeleportDelay + 0.5) -- kasih delay tambahan biar fix posisinya

            -- 3. Cek summit
            if char and char:FindFirstChild("HumanoidRootPart") then
                local pos = char.HumanoidRootPart.Position
                if IsAtPuncak(pos, 15) then -- threshold lebih longgar, tapi aman
        ...
        -- 4. Respawn
        respawnCharacter()
        summitReached = false -- reset flag untuk summit berikutnya
        task.wait(2)
    end
end
