...
-- =========================================
-- Teleport ke Checkpoint Berikutnya (Fun Tab)
-- =========================================
local nextCPIndex = 1 -- indeks checkpoint saat ini

FunTab:CreateButton({
    Name = "Teleport ke Checkpoint Berikutnya",
    Callback = function()
        local Players = game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
        ...
        if #cps == 0 then
            Rayfield:Notify({
                ...
                Content = "Tidak ada checkpoint terdeteksi di server ini.",
                Duration = 5,
            })
            return
        end

        -- Reset indeks jika sudah mencapai akhir
        if nextCPIndex > #cps then
            nextCPIndex = 1
        end

        local cp = cps[nextCPIndex]
        local char = LocalPlayer.Character
        if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        Rayfield:Notify({
            Title = "Teleport Checkpoint",
            Content = "Teleport ke checkpoint " .. nextCPIndex .. " berhasil!",
            Duration = 4,
        })

        nextCPIndex += 1 -- naikkan indeks untuk klik berikutnya
    end,
})
