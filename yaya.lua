...
-- ✅ Tambahkan ke Rayfield (misalnya di FunTab)
FunTab:CreateToggle({
    Name = "Duduk di Player Terdekat",
    CurrentValue = false,
    Flag = "SitOnPlayer",
    Callback = function(Value)
        sitActive = Value
        if sitActive then
            ...
            if target then
                ...
                Rayfield:Notify({
                    Title = "Aktif",
                    Content = "keplayer " .. target.Name,
                    Duration = 3
                })
            else
                Rayfield:Notify({
                    Title = "Tidak Ada Target",
                    Content = "Tidak ada player terdekat.",
                    Duration = 3
                })
            end
        else
            ...
            Rayfield:Notify({
                Title = "Nonaktif",
                Content = "Kamu turun dari player.",
                Duration = 3
            })
        end
    end,
...
-- Misal Tab sudah ada:
-- local Tab = Window:CreateTab("CP Controls", 4483362458)

-- Event checkpoint
local CPEvent = game:GetService("ReplicatedStorage"):WaitForChild("CheckpointEvent")

-- Toggle Auto-Bypass CP
FunTab:CreateToggle({
    Name = "Auto Bypass CP",
    CurrentValue = false,
    Flag = "AutoBypass",
    Callback = function(value)
        -- Nilai toggle otomatis digunakan di loop
    end
})

-- Daftar CP yang ingin di-bypass (misal CP2 sampai CP5)
local CPIndexes = {2,3,4,5}

-- Loop untuk auto bypass, tempel di script yang sama
task.spawn(function()
    while true do
        if Tab.Flags.AutoBypass then -- cek toggle aktif
            for _, index in pairs(CPIndexes) do
                pcall(function()
                    if CPEvent then
                        CPEvent:FireServer(index) -- bypass CP tanpa teleport
                    end
                end)
            end
        end
        task.wait(1) -- loop tiap 1 detik
    end
end)
