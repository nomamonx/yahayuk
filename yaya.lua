...
local AutoClimbTask = nil

-- Fungsi scan checkpoint otomatis
local function GetCheckpoints()
    local cps = {}

    -- Scan semua part / model di workspace
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and string.match(obj.Name:lower(), "cp") then
            table.insert(cps, obj)
        end
    end

    -- Urutkan berdasarkan tinggi (Y)
...
local function StartAutoClimb()
    if AutoClimbEnabled then return end
    AutoClimbEnabled = true
    AutoClimbTask = task.spawn(function()
        local checkpoints = GetCheckpoints()
        if #checkpoints == 0 then
            warn("Tidak ada checkpoint terdeteksi di server ini.")
            AutoClimbEnabled = false
            return
        end

        local index = 1
        while AutoClimbEnabled and index <= #checkpoints do
            local cp = checkpoints[index]
            TeleportToCheckpoint(cp)

            -- Tunggu sampai karakter mendekati checkpoint (jarak < 5)
            repeat
                task.wait(0.5)
                local char = LocalPlayer.Character
                if char and char:FindFirstChild("HumanoidRootPart") then
                    local dist = (cp.Position - char.HumanoidRootPart.Position).Magnitude
                    if dist <= 5 then break end
                end
            until not AutoClimbEnabled
...
-- Fungsi Stop Auto Climb
local function StopAutoClimb()
    AutoClimbEnabled = false
    if AutoClimbTask then
        task.cancel(AutoClimbTask)
        AutoClimbTask = nil
    end
end

-- Tambahkan ke Tab Teleport
TeleportTab:CreateToggle({
    Name = "Auto Climb Checkpoints",
    CurrentValue = false,
