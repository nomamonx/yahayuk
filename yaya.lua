...
-- Refresh daftar player (DisplayName)
local function RefreshPlayerList()
    local names = {}
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
...
-- Tombol nempel dan duduk
FunTab:CreateButton({
    Name = "Nempel & Duduk Sedih",
    Callback = function()
        if not targetPlayer or not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
            Rayfield:Notify({Title="Error", Content="Target tidak valid!", Duration=5})
            return
        end

        local char = LocalPlayer.Character
        if not char or not char:FindFirstChild("HumanoidRootPart") or not char:FindFirstChild("Humanoid") then
            Rayfield:Notify({Title="Error", Content="Karakter kamu tidak valid!", Duration=5})
            return
        end
        ...
        humanoid:LoadAnimation(workspace:WaitForChild("Emotes"):WaitForChild("Sedih"):WaitForChild("Sit")):Play() -- contoh emote duduk sedih, sesuaikan path

        -- AlignPosition untuk menempel ke HumanoidRootPart target
        ...
        -- AlignOrientation supaya karakter menghadap target
        local alignOri = Instance.new("AlignOrientation")
        alignOri.MaxTorque = 1e6
        alignOri.Responsiveness = 50
        alignOri.RigidityEnabled = true
        local attOri = Instance.new("Attachment", hrp)
        alignOri.Attachment0 = attOri
        alignOri.Parent = hrp

        attachObjects.alignPos = alignPos
        attachObjects.alignOri = alignOri
        attachObjects.att0 = att0
        attachObjects.attOri = attOri
        attachObjects.target = targetPlayer

        attachObjects.updateLoop = RunService.RenderStepped:Connect(function()
            if attachObjects.target and attachObjects.target.Character and attachObjects.target.Character:FindFirstChild("HumanoidRootPart") then
                local targetHRP = attachObjects.target.Character.HumanoidRootPart
                attachObjects.alignPos.Position = targetHRP.Position
            end
        end)

        Rayfield:Notify({Title="Fun", Content="Sekarang nempel & duduk sedih di "..targetPlayer.DisplayName, Duration=5})
    end,
})

-- Tombol berhenti nempel
FunTab:CreateButton({
    Name = "Stop Nempel",
    Callback = function()
        local char = LocalPlayer.Character
        ...
        if attachObjects.updateLoop then
            attachObjects.updateLoop:Disconnect()
            attachObjects.updateLoop = nil
        end
        if attachObjects.alignPos then attachObjects.alignPos:Destroy() attachObjects.alignPos=nil end
        if attachObjects.alignOri then attachObjects.alignOri:Destroy() attachObjects.alignOri=nil end
        if attachObjects.att0 then attachObjects.att0:Destroy() attachObjects.att0=nil end
        if attachObjects.attOri then attachObjects.attOri:Destroy() attachObjects.attOri=nil end
        attachObjects.target = nil

        Rayfield:Notify({Title="Fun", Content="Berhenti nempel & duduk.", Duration=5})
    end,
})
