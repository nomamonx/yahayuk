...
local function StartFly()
    ...
    flyConn = RunService.RenderStepped:Connect(function(dt)
        ...
        local input = UserInputService:GetFocusedTextBox() == nil -- biar gak ganggu chat

        -- Ambil input WASD/Analog
        if input then
            local forward = Camera.CFrame.LookVector
            local right = Camera.CFrame.RightVector

            -- Normalisasi agar cuma XZ
            ...
            -- Mobile thumbstick (analog bawaan Roblox)
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
                moveDir += LocalPlayer.Character:FindFirstChildOfClass("Humanoid").MoveDirection
            end
...
-- ========================
-- Toggle & Slider di Tab Baru
-- ========================
local FlyTab = Window:CreateTab("Fly", 4483362458) -- ganti icon ID sesuai selera
