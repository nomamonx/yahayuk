...
FunTab:CreateButton({
    ...
    Callback = function()
        ...
        -- Mainkan animasi duduk sedih (sesuaikan path)
        local anim = Instance.new("Animation")
        anim.AnimationId = "rbxassetid://507770239" -- Contoh duduk sedih, ganti sesuai game
        ...
        attachObjects = {alignPos=alignPos, alignOri=alignOri, att0=att0, attOri=attOri, target=targetPlayer, track=track}

        -- Update loop untuk mengikuti target
        attachObjects.updateLoop = RunService.RenderStepped:Connect(function()
            if attachObjects.target and attachObjects.target.Character and attachObjects.target.Character:FindFirstChild("HumanoidRootPart") then
                ...
                attachObjects.alignPos.Position = targetHRP.Position + Vector3.new(0,3,0) -- offset 3 studs di atas
            end
        end)
...
FunTab:CreateButton({
    ...
    Callback = function()
        ...
        if attachObjects.track then attachObjects.track:Stop() attachObjects.track=nil end
