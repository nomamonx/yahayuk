...
-- Toggle Fall Damage
local FallDamageEnabled = true -- default aktif
SettingsTab:CreateToggle({
    Name = "No Fall Damage",
    CurrentValue = false,
    Flag = "NoFallDamage",
    Callback = function(Value)
        FallDamageEnabled = not Value
    end,
})

-- Matikan fall damage saat HumanoidHealthChanged
LocalPlayer.CharacterAdded:Connect(function(char)
    local humanoid = char:WaitForChild("Humanoid")
    humanoid.StateChanged:Connect(function(_, newState)
        if newState == Enum.HumanoidStateType.FallingDown and not FallDamageEnabled then
            humanoid:ChangeState(Enum.HumanoidStateType.Physics)
        end
    end)
end)

-- Toggle Respawn di Tempat Mati
local RespawnAtDeathEnabled = false
local LastPosition = nil

-- Update posisi terakhir setiap frame
...
-- Saat karakter mati, spawn di posisi terakhir
LocalPlayer.CharacterAdded:Connect(function(char)
    if RespawnAtDeathEnabled and LastPosition then
        local humanoidRootPart = char:WaitForChild("HumanoidRootPart")
        humanoidRootPart.CFrame = CFrame.new(LastPosition + Vector3.new(0,3,0))
    end
end)
