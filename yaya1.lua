...
-- ========================
-- FunTab: Follow + Joget + Kepala Terbalik
-- ========================
local Players = game:GetService("Players")
...
local JogetAmplitude = 1 -- seberapa lebar gerakan joget
local JogetSpeed = 5 -- kecepatan joget
...
-- Toggle Follow + Joget
local FollowJogetEnabled = false
FunTab:CreateToggle({
    Name = "Follow + Joget Player",
    CurrentValue = false,
    Flag = "FollowJogetToggle",
    Callback = function(value)
        FollowJogetEnabled = value

        if FollowJogetEnabled then
            local t = 0
            FollowConnection = RunService.Heartbeat:Connect(function(dt)
                ...
                if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    local char = LocalPlayer.Character
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        -- Posisi ngejar target + joget kiri-kanan
                        local offset = Vector3.new(math.sin(t) * JogetAmplitude, 0, math.cos(t) * JogetAmplitude)
...
-- Toggle Kepala di bawah, Kaki di atas
local InvertEnabled = false
FunTab:CreateToggle({
    Name = "Kepala di bawah, Kaki di atas",
    CurrentValue = false,
    Flag = "InvertToggle",
    Callback = function(value)
        InvertEnabled = value
        local char = LocalPlayer.Character
        if char and char:FindFirstChildOfClass("HumanoidRootPart") then
            if InvertEnabled then
                char.HumanoidRootPart.CFrame *= CFrame.Angles(math.rad(180),0,0)
            else
