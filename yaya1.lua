...
-- Cek posisi puncak setiap frame
local function OnReachPuncak()
    local char = LocalPlayer and LocalPlayer.Character
    if not (char and char:FindFirstChild("HumanoidRootPart")) then return end

    local pos = char.HumanoidRootPart.Position
    if (pos - teleportPoints.Puncak.Position).Magnitude <= PuncakRadius then
        if not LocalPlayer.HasReachedPuncak then
            LocalPlayer.HasReachedPuncak = true
            Log("Player sampai puncak, hitung summit dalam " .. SummitDelay .. " detik")
            task.delay(SummitDelay, OnRespawn) -- delay sesuai slider
        end
    end
end
...
function OnRespawn()
    ...
    task.spawn(function()
        ...
        LocalPlayer.HasReachedPuncak = false
        ...
        if Config.NotifyInGame then 
            Notify("🏔️ Summit", "Summit dihitung! Total: " .. tostring(summitStats.count))
        end

        Log("Summit dihitung:", summitStats.count, "time:", summitStats.lastTime)
...
local function HookRespawn(char)
    ...
    humanoid.Died:Connect(function()
        ...
        if LocalPlayer.HasReachedPuncak then
            task.delay(SummitDelay, OnRespawn)
        end
        print("Karakter respawn")
    end)
end

-- Setiap karakter baru muncul
LocalPlayer.CharacterAdded:Connect(HookRespawn)

-- Kalau karakter sudah ada saat skrip jalan
if LocalPlayer.Character then
    HookRespawn(LocalPlayer.Character)
end

-- Heartbeat watcher
RunService.Heartbeat:Connect(OnReachPuncak)

-- Heartbeat watcher baru untuk cek puncak
RunService.Heartbeat:Connect(function()
    local char = LocalPlayer.Character
    if char and char:FindFirstChild("HumanoidRootPart") then
        local dist = (char.HumanoidRootPart.Position - teleportPoints.Puncak.Position).Magnitude
        print("Jarak ke puncak:", dist)
        if dist <= PuncakRadius then
            print("Sudah sampai puncak")
            if not LocalPlayer.HasReachedPuncak then
                LocalPlayer.HasReachedPuncak = true
                task.delay(SummitDelay, OnRespawn)
            end
