    ...
    Callback = function(value)
        ...
        if DanceEnabled then
            ...
            DanceConnection = RunService.Heartbeat:Connect(function()
                ...
                local motors = {
                    getMotor("LeftUpperArm"),
                    getMotor("RightUpperArm"),
                    getMotor("LeftUpperLeg"),
                    getMotor("RightUpperLeg")
                }

                for i, motor in ipairs(motors) do
                    if motor then
                        if i <= 2 then
                            motor.C0 = motor.C0:Lerp(CFrame.Angles(angle,0,0),0.5)
                        else
                            motor.C0 = motor.C0:Lerp(CFrame.Angles(-angle,0,0),0.5)
                        end
                    end
            ...
            local char = LocalPlayer.Character
            if char then
                for _, partName in ipairs({"LeftUpperArm","RightUpperArm","LeftUpperLeg","RightUpperLeg"}) do
                    local part = char:FindFirstChild(partName)
                    if part then
                        for _, m in ipairs(part:GetChildren()) do
                            if m:IsA("Motor6D") then
