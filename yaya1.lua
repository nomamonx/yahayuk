...
local DistanceBack = 8   -- jarak kamera di belakang target
local Height = 5         -- tinggi kamera dari HumanoidRootPart
local Smooth = 0.3       -- lerp smooth
local RefreshRate = 0.03 -- update tiap frame
    ...
    Callback = function(opt)
        for _, p in ipairs(Players:GetPlayers()) do
            if p.DisplayName == opt[1] then
                SpectateTarget = p
                -- langsung teleport ke target
                if p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    local root = p.Character.HumanoidRootPart
                    local lookVec = root.CFrame.LookVector
                    Camera.CFrame = CFrame.new(root.Position - lookVec * DistanceBack + Vector3.new(0, Height, 0), root.Position)
                end
                break
            end
        end
...
SettingsTab:CreateToggle({
    ...
    Callback = function(val)
        ...
        if SpectateEnabled then
            ...
            -- teleport instan ke target jika ada
...
RunService.RenderStepped:Connect(function()
    if SpectateEnabled and SpectateTarget and SpectateTarget.Character and SpectateTarget.Character:FindFirstChild("HumanoidRootPart") then
        local root = SpectateTarget.Character.HumanoidRootPart
        local lookVec = root.CFrame.LookVector
        local targetPos = root.Position - lookVec * DistanceBack + Vector3.new(0, Height, 0)
        Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(targetPos, root.Position), Smooth)
    end
end)

-- ======= Auto refresh dropdown saat player join/leave =======
