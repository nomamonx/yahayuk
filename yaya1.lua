...
if not success or not RayfieldOrErr then
    print("Rayfield gagal dimuat. Pastikan URL dapat diakses atau executor-mu mendukung HttpGet.")
    if typeof(RayfieldOrErr) == "string" then
        print("Error detail:", RayfieldOrErr)
    end
    return
end
...
end -- <- tambahkan end di sini untuk menutup if isVIP

-- ===============================
local DataFolder = "Detektoryaha"
local DataFile = "SummitStats.json"

local function SaveSummitStats()
    pcall(function()
        if not isfolder(DataFolder) then
            makefolder(DataFolder)
        end
        writefile(DataFolder.."/"..DataFile, HttpService:JSONEncode(summitStats))
    end)
end

local function LoadSummitStats()
    if isfile(DataFolder.."/"..DataFile) then
        local data = readfile(DataFolder.."/"..DataFile)
...
LoadSummitStats() -- load data summit saat script start
...
local function ScanAllPlayers()
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl ~= LocalPlayer then
            print("Mendeteksi player:", pl.Name)
            CheckPlayerForAdminEnhanced(pl)
        end
    end
end
...
task.spawn(function()
    while true do
        task.wait(Config.ScanInterval or 5)
        ScanAllPlayers()
    end
end)
...
-- Tambahin @everyone di awal pesan
local msg = ("@everyone\n⚠️ Admin Detected: **%s** (%s)\nReason: %s\nStatus: %s\nPlaceId: %d\nTime: %s"):format(
...
pcall(function()
    SendWebhook(("⚠️ @everyone Admin Detected — %s"):format(pname), msg, {
        color = 15158332,
        fields = {
...
-- Default delay (detik)
local SummitDelay = 3
local PuncakRadius = 10
local lastSummitNotify = 0
local summitProcessing = false

-- Load/save settings flag
if not Config then Config = {} end
if Config.SummitDelay == nil then
    Config.SummitDelay = SummitDelay
end
...
SettingsTab:CreateSlider({
    ...
    Increment = 1,
    Suffix = "s",
    CurrentValue = Config.SummitDelay,
    Flag = "SummitDelay",
    Callback = function(value)
        Config.SummitDelay = value
        SummitDelay = value
        Log("Summit delay diubah:", value, "detik")
    end
})


-- =========================
-- Summit Detection Plug & Play
-- =========================

local SummitDelay = Config.SummitDelay or 3
local PuncakRadius = 10
local summitProcessing = false
local lastSummitNotify = 0
local summitStats = summitStats or { count = 0, lastTime = 0, lastRespawn = tick() }
...
local function OnSummitReached()
    if summitProcessing then return end
    summitProcessing = true
    task.spawn(function()
        local now = tick()
        if now - lastSummitNotify < 1 then
            summitProcessing = false
            return
        end
        lastSummitNotify = now
        ...
        local timeTaken = math.floor(now - (summitStats.lastRespawn or now))
        summitStats.lastTime = timeTaken
        summitStats.count = (summitStats.count or 0) + 1
        summitStats.lastRespawn = now

        if Config.NotifyInGame then
            Rayfield:Notify({
                Title = "🏔️ Summit Reached",
                Content = "Total Summit: "..summitStats.count.."\nTime: "..FormatTimeSec(timeTaken),
                Duration = 4
            })
        end

        if Config.SendWebhook and Config.WebhookURL and Config.WebhookURL ~= "" then
            pcall(function()
                local body = HttpService:JSONEncode({
                    content = ("✅ Summit Reached by %s! Total: %d, Time: %ds"):format(player.Name, summitStats.count, timeTaken)
                })
                pcall(function() game:HttpPost(Config.WebhookURL, body, Enum.HttpContentType.ApplicationJson) end)
            end)
        end
         SaveSummitStats()   
        summitProcessing = false
...
task.spawn(function()
    local player = game.Players.LocalPlayer
    repeat task.wait(0.5) until player:FindFirstChild("leaderstats") and player.leaderstats:FindFirstChild("Summit")
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        if (hrp.Position - teleportPoints.Puncak.Position).Magnitude <= PuncakRadius then
            positionHoldTimer = positionHoldTimer + dt
...
-- Heartbeat watcher baru untuk cek puncak
