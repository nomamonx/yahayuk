                ...
                while isAutoTeleporting do
                    for i, cf in ipairs(autoTeleportPoints) do
                        ...
                        if TeleportCounter >= TeleportLimitPerCycle then
                         ...
                         -- Fungsi hook listener
                    local LocalPlayer = game.Players.LocalPlayer
                    local playerGui = LocalPlayer:WaitForChild("PlayerGui")
                    local Rayfield = Rayfield or {} -- kalau Rayfield nggak ada, biar nggak error
                    Rayfield.Notify = Rayfield.Notify or function(tbl) print(tbl.Title, tbl.Content) end

                    local SavedSummitButton = nil

            -- Fungsi hook listener
                local function hookButton(btn)
                if btn:IsA("TextButton") or btn:IsA("ImageButton") then
                btn.MouseButton1Click:Connect(function()
            -- Debug: notif tiap tombol yang diklik
               Rayfield:Notify({
                Title = "🖱️ Button Clicked",
                Content = "Kamu klik: " .. (btn.Name or "Unknown"),
                Duration = 3
            })
            
            -- Kalau nama tombol ada kata "Puncak", simpan buat auto klik
            if string.lower(btn.Name):find("puncak") then
                SavedSummitButton = btn
                Rayfield:Notify({
                    Title = "✅ Summit Button Terdeteksi",
                    Content = "Disimpan untuk auto klik selanjutnya!",
                    Duration = 4
                })
            end
        end)
...
-- Hook semua tombol yang sudah ada
for _, obj in ipairs(playerGui:GetDescendants()) do
    hookButton(obj)
end

-- Hook tombol baru yang muncul
