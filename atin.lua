...
local function IsAtPuncak(pos, threshold)
    ...
    return (pos - peakPos).Magnitude <= threshold
end
...
local function AutoTeleportLoop()
    ...
    while isAutoTeleporting do
        ...
        -- 3. Cek bener-bener di Puncak
        if char and char:FindFirstChild("HumanoidRootPart") then
            if IsAtPuncak(char.HumanoidRootPart.Position, 10) then
                OnSummitReachedEnhanced() -- notif hanya sekali & hanya di Puncak
            end
        end
