...
local function OnSummitReachedEnhanced()
   if summitReached then return end -- cegah spam
    ...
    -- 🔄 Reset cache server tiap 5 summit
    if summitStats.count % 5 == 0 then
        bannedServersCache = {}
        Log("🔄 Cache server direset setelah 5 summit.")
    end
end


local summitReached = false

local function IsAtPuncak(pos, threshold)
    local peakPos = teleportPoints.Puncak.Position
    return (pos - peakPos).Magnitude <= threshold and pos.Y > 2100
end
...
local function AutoTeleportLoop()
    ...
    while isAutoTeleporting do
        ...
        task.wait(1.5) -- kasih waktu pindah dulu

        -- Tunggu sampai benar-benar di Puncak
        ...
        while waited < maxWait do
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                if not summitReached and IsAtPuncak(pos, 20) then
                    OnSummitReachedEnhanced()
                    respawnCharacter()
                    summitReached = false -- reset setelah respawn
                    break
                end
            end
        end
...
function serverHopAttemptEnhanced()
    ...
    for page = 1, maxPages do
        ...
        for _, srv in ipairs(json.data or {}) do
            ...
            if type(srv) == "table" and srv.id and srv.playing and srv.maxPlayers then
                if tostring(srv.id) ~= tostring(JobId)
                    and (srv.playing < srv.maxPlayers)
                    and (not bannedServersCache[srv.id]) then
                    found = srv.id
                    break
                end
            end
    ...
    if found then
        ...
        if not success then
            ...
            -- safety check: kalau 5 detik masih di server lama, hop ulang
            local oldJob = game.JobId
            task.delay(5, function()
                if game.JobId == oldJob then
                    Notify("Hop Retry", "Masih di server lama, coba hop ulang...")
                    TeleportService:Teleport(PlaceId, LocalPlayer)
                end
            end)
        end
...
local function OnSummitReachedEnhanced()
      -- cegah spam, hanya sekali per loop summit
    if summitReached then return end
    summitReached = true  
    ...
    -- webhook notif (hanya sekali per summit)
