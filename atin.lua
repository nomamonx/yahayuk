...
task.wait(AutoTeleportDelay + 1) -- kasih delay biar fix posisi

-- Cek benar-benar di Puncak
if char and char:FindFirstChild("HumanoidRootPart") then
    local pos = char.HumanoidRootPart.Position
    if not summitReached and (pos - teleportPoints.Puncak.Position).Magnitude <= 20 and pos.Y > 2100 then
        summitReached = true
        OnSummitReachedEnhanced()
        respawnCharacter()
        summitReached = false -- reset supaya summit berikutnya bisa dihitung
    end
end
