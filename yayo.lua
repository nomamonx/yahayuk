...
local function applyNoName(char)
    ...
    -- matikan name bawaan Roblox
    humanoid.NameDisplayDistance = 0
    humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
    humanoid.DisplayName = "" -- kosongin biar ga muncul

    -- hapus semua Billboard bawaan di head (sekali aja)
    local head = char:WaitForChild("Head", 5)
    if head then
        for _, gui in ipairs(head:GetChildren()) do
            if gui:IsA("BillboardGui") then
                gui.Enabled = false -- disable (bukan destroy, biar ga spawn ulang kedip)
            end
        end

        -- bikin fake name sendiri
