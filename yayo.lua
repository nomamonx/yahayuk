...
local CameraOffset = Vector3.new(0, 2, 6) -- kamera di atas & belakang
local SmoothSpeed = 0.1 -- kecepatan smooth
...
RunService.RenderStepped:Connect(function(deltaTime)
    if SpectateEnabled and SpectateTarget and SpectateTarget.Character and SpectateTarget.Character:FindFirstChild("HumanoidRootPart") then
        local targetPos = SpectateTarget.Character.HumanoidRootPart.Position
        local desiredCFrame = CFrame.new(targetPos + CameraOffset, targetPos)
        Camera.CFrame = Camera.CFrame:Lerp(desiredCFrame, SmoothSpeed)
    end
end)

-- Refresh dropdown saat player baru join
Players.PlayerAdded:Connect(function()
    PlayerDropdown:Refresh(RefreshPlayerList(), true)
end)

-- Tetap spectate saat target respawn
Players.PlayerRemoving:Connect(function(plr)
...
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function()
        if SpectateEnabled and SpectateTarget == plr then
            -- otomatis lanjut spectate
        end
    end)
end)
