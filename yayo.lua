...
function serverHopAttemptEnhanced()
    ...
    for page = 1, maxPages do
        ...
        local ok2, json = pcall(HttpService.JSONDecode, HttpService, res)
        if not ok2 or type(json) ~= "table" then
            Log("serverHopAttemptEnhanced: JSON decode failed at page", page)
            break
        end
        ...
        for _, srv in ipairs(json.data or {}) do
            ...
            if type(srv) == "table" and srv.id and srv.playing and srv.maxPlayers then
                Log("Checking server:", srv.id, "Playing:", srv.playing, "Max:", srv.maxPlayers)
    ...
    if found then
        ...
        -- Spawn teleport di dalam pcall agar error tidak mengganggu
        task.spawn(function()
            ...
            if not success then
                ...
                bannedServersCache[found] = true -- tandai server gagal
            end
        end)
        return true
    else
