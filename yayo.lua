...
-- ========================
-- FunTab: Follow + Sit Player (duduk goyang, tinggi bisa diatur)
-- ========================
local Players = game:GetService("Players")
...
local SitEmote = "rbxassetid://1347720070" -- ID animasi duduk
local OffsetY = 1.5 -- default tinggi (lebih tinggi dari sebelumnya)
local OscillationAmplitude = 0.3 -- sejauh mana karakter bergoyang
local OscillationSpeed = 3 -- kecepatan goyang
...
-- Fungsi mainkan emote duduk
local function PlaySitEmote()
    local char = LocalPlayer.Character
    if char and char:FindFirstChildOfClass("Humanoid") then
        ...
        anim.AnimationId = SitEmote
        local track = animator:LoadAnimation(anim)
        track:Play()
    end
end

-- Toggle Follow + Sit + Goyang
local FollowSitEnabled = false
FunTab:CreateToggle({
    Name = "Follow + Sit Player",
    CurrentValue = false,
    Flag = "FollowSitToggle",
    Callback = function(value)
        FollowSitEnabled = value

        if FollowSitEnabled then
            PlaySitEmote()
            local t = 0
            FollowConnection = RunService.Heartbeat:Connect(function(dt)
                t += dt * OscillationSpeed
                if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    local char = LocalPlayer.Character
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        -- Posisi ngejar target + goyang
                        local offset = Vector3.new(math.sin(t) * OscillationAmplitude, OffsetY, math.cos(t) * OscillationAmplitude)
                        char.HumanoidRootPart.CFrame = CFrame.new(TargetPlayer.Character.HumanoidRootPart.Position + offset)
                    end
                end
            end)
        else
...
-- Toggle Pilih Tinggi / Rendah
FunTab:CreateToggle({
    Name = "Duduk Tinggi / Rendah",
    CurrentValue = true,
    Flag = "FollowOffsetToggle",
    Callback = function(value)
        if value then
            OffsetY = 1.5 -- tinggi, di atas HumanoidRootPart target
        else
            OffsetY = 1 -- rendah, lebih dekat badan
        end
    end
})
