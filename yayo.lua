...
local Config = {
    ...
    WebhookURL = "https://discord.com/api/webhooks/1314381124557865010/rjde-YwMH6pOi9Dk7LzQhlbCg1RGhYvouHgwrz_dYJi8amlIQLImuHnRXlLot-1mFfUU", -- default placeholder
    ...
    HWIDCheckURL = "https://raw.githubusercontent.com/nomamonx/gunung/refs/heads/main/hwid1.lua", -- optional
}
...
do
    local ok, res = pcall(function()
        ...
        -- try to fetch remote HWID table (optional)
        local successFetch, text = pcall(function() return game:HttpGet(Config.HWIDCheckURL) end)
        if successFetch and text then
            local ok2, tbl = pcall(function() return loadstring(text)() end)
    ...
    if not ok then
        Log("HWID check error:", res)
    end
end
local isVIP = (hwid and AllowedHWIDs[hwid]) == true
...
local TabHWID = Window:CreateTab("HWID")
...
local SummitLabel = CreateLabel(TabSummit, "Summit: 0 | Last: 00m:00s")
local btnResetSummit = CreateButton(TabSummit, { Name = "♻️ Reset Summit Stats", Callback = function()
    summitStats.count = 0
...
local btnTestWebhook = CreateButton(TabSettings, { Name = "📡 Test Webhook", Callback = function()
    local ok, err = SendWebhook("✅ Test Webhook", "Ini pesan test dari Detektor. Jika tidak muncul, cek webhook dan executor.")
    if ok then Notify("Webhook", "Test terkirim!") else Notify("Webhook Gagal", tostring(err)) end
end })
...
if hwid then
    if isVIP then
        CreateParagraph(TabHWID, { Title = "✅ HWID Terdaftar", Content = "HWID kamu sudah terdaftar. ID:\n\n"..tostring(hwid) })
    else
        CreateParagraph(TabHWID, { Title = "❌ HWID Tidak Terdaftar", Content = "HWID kamu tidak terdaftar. ID:\n\n"..tostring(hwid) })
    end
    CreateButton(TabHWID, { Name = "↪️ Salin HWID", Callback = function() CopyToClipboard(tostring(hwid)) end })
end
...
function serverHopAttemptEnhanced()
    ...
    if found then
        ...
        Log("No suitable server found to hop.")
        return false
    end
end
