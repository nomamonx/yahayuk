...
-- ============================================
-- Universal Fly (PC & Mobile, Physics Fix)
-- ============================================
...
local function startFly()
    ...
    flying = true
    hum:ChangeState(Enum.HumanoidStateType.Physics) -- matikan physics default
    ...
    flyConn = RunService.RenderStepped:Connect(function(dt)
        ...
        if UserInputService.KeyboardEnabled then
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then
                moveDir += Vector3.new(camCF.LookVector.X, 0, camCF.LookVector.Z)
            end
        ...
        elseif UserInputService.TouchEnabled then
            ...
            if hum then
                ...
                moveDir += Vector3.new(dir.X, 0, dir.Z)
            end
            if upPressed then
                moveDir += Vector3.new(0,1,0)
            end
...
local function stopFly()
    ...
    if char then
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            hum:ChangeState(Enum.HumanoidStateType.GettingUp) -- balikin normal
        end
    end
end

SettingsTab:CreateToggle({
    Name = "Fly (Universal Fix)",
    CurrentValue = false,
    Flag = "FlyToggleUniversalFix",
    Callback = function(Value)
        if Value then
            startFly()
        else
