...
-- ========================
-- Sensor Nama Saya (Local Only)
-- ========================
local Players = game:GetService("Players")
local lp = Players.LocalPlayer
local randomActive = false

-- daftar nama palsu (boleh edit sesuka hati)
local fakeNames = {
    "OrangMisterius","PlayerX","NoobBergaya","SiTembus","HantuGunung"
}

-- ambil nama random dari list
local function getRandomName()
    return fakeNames[math.random(1, #fakeNames)]
end

-- apply sensor nama asli + pasang fake name
local function applyNoName(char)
    local humanoid = char:WaitForChild("Humanoid", 5)
    local head = char:WaitForChild("Head", 5)
    if not humanoid or not head then return end

    -- matikan label nama bawaan Roblox
    humanoid.NameDisplayDistance = 0
    humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
    humanoid.DisplayName = "" -- kosong biar aman
...
local function resetName(char)
    ...
    if humanoid then
        ...
        humanoid.DisplayName = lp.Name -- balikin nama asli
    end
end
