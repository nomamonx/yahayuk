...
local AutoBypassToggle = FunTab:CreateToggle({
    Name = "Auto Bypass CP",
    CurrentValue = false,
    Flag = "AutoBypass",
    Callback = function(value)
        -- toggle otomatis akan tersedia di FunTab.Flags.AutoBypass
    end
})

-- Daftar CP yang ingin di-bypass
local CPIndexes = {2,3,4,5}

-- Loop untuk auto bypass
task.spawn(function()
    while true do
        if FunTab.Flags.AutoBypass then
            for _, index in pairs(CPIndexes) do
                pcall(function()
                    if CPEvent then
                        CPEvent:FireServer(index)
                    end
                end)
            end
        end
