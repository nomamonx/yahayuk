...
-- Tombol duduk di kepala
FunTab:CreateButton({
    Name = "Duduk di Kepala Pemain",
    Callback = function()
        ...
        -- Matikan kontrol Humanoid sementara
        humanoid.PlatformStand = true

        -- AlignPosition untuk menempel di atas kepala target
        ...
        attachObjects.updateLoop = RunService.RenderStepped:Connect(function()
            if attachObjects.target and attachObjects.target.Character and attachObjects.target.Character:FindFirstChild("Head") then
                local targetHead = attachObjects.target.Character.Head
                attachObjects.alignPos.Position = targetHead.Position + Vector3.new(0,2,0)
            end
        end)

        Rayfield:Notify({Title="Fun", Content="Sekarang kamu duduk di kepala "..targetPlayer.DisplayName, Duration=5})
    end,
})
...
FunTab:CreateButton({
    ...
    Callback = function()
        ...
        if attachObjects.alignPos then
            attachObjects.alignPos:Destroy()
            attachObjects.alignPos = nil
        end
        if attachObjects.alignOri then
            attachObjects.alignOri:Destroy()
            attachObjects.alignOri = nil
        end
        ...
        Rayfield:Notify({Title="Fun", Content="Kamu berhenti duduk di kepala pemain.", Duration=5})
    end,
})
