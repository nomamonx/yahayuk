...
-- ========================
-- Fly dengan UI Tombol Besar (PC + HP)
-- ========================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local LocalPlayer = Players.LocalPlayer
...
local function StartFly()
    ...
    flyConn = RunService.RenderStepped:Connect(function()
        ...
        hrp.Velocity = Vector3.zero -- default diam, nanti diganti tombol
    end)
end
...
-- Tombol UI Besar
local FlyControls = FlyTab:CreateSection("Kontrol Fly")

local btnUp = FlyTab:CreateButton({
    Name = "⬆ Naik",
    Callback = function()
...
local btnDown = FlyTab:CreateButton({
    Name = "⬇ Turun",
    Callback = function()
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp and flying then
...
local btnStop = FlyTab:CreateButton({
    Name = "⏹ Berhenti",
    Callback = function()
        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
        if hrp then
