...
local TeleportLimitPerCycle = 120  -- max teleport sebelum pause
                ...
                while isAutoTeleporting do
                    for i, cf in ipairs(autoTeleportPoints) do
                        ...
                        if TeleportCounter >= TeleportLimitPerCycle then
                            Rayfield:Notify({
                                Title = "Auto Teleport",
                                Content = "Teleport limit tercapai, pause "..AutoTeleportPauseTime.."s...",
                                Duration = 3,
                            })
                    ...
                    repeat
                        ...
                        local char = player.Character
                        if char and char:FindFirstChild("HumanoidRootPart") then
                            if (char.HumanoidRootPart.Position - teleportPoints.Spawn.Position).Magnitude <= 10 then
                                break
                            end
                        end
...
local function performEscapeAction(playerName, reason)
    if Config.AutoHop then
        local ok, err = pcall(serverHopAttempt)
        if ok then
            SendWebhook(
                "Admin Detected - Attempting Hop",
                ("Detected **%s** (%s). Attempting server hop."):format(playerName, reason),
                { color = 16766720 }
            )
            Notify("AutoHop", "Mencoba hop server untuk admin: " .. playerName)
            return true
        else
            Log("AutoHop attempt failed for", playerName, err)
...
local function handleAdminDetectedEnhanced(player, reason)
    ...
    -- pastikan reason string, bukan boolean
    reason = reason or CheckPlayerForAdminEnhanced(player) or "Unknown"
    ...
    -- update GUI
    pcall(function()
        AdminLabel:Set("Admin Terdeteksi: " .. pname .. " (" .. reason .. ")")
        StatusLabel:Set("Status: Admin terdeteksi - " .. pname)
    end)
    ...
    if Config.NotifyInGame then
        Notify("⚠️ Admin Terdeteksi", pname .. " (" .. reason .. ")")
    end
    pcall(function() CopyToClipboard(pname) end)

    -- jalankan escape (hop/leave) setelah delay kecil
    task.spawn(function()
        task.wait(0.6)
...
function CheckPlayerForAdminEnhanced(player)
    ...
    -- 1️⃣ check blacklist
    local ok, reason = pcall(isBlacklisted, player)
    if ok and reason then return reason end

    -- 2️⃣ check rank keywords
    local ok2, reason2 = pcall(hasRankKeywordInName, player)
    if ok2 and reason2 then return reason2 end

    -- 3️⃣ check GUI
    local ok3, reason3 = pcall(guiAdminDetect, player)
    if ok3 and reason3 then return reason3 end

    -- 4️⃣ heuristics: unusual Humanoid properties
    pcall(function()
        if player.Character and player.Character:FindFirstChild("Humanoid") then
            ...
            if ws > 60 or jp > 220 then
                return ("UnusualMovement(ws=%s,jp=%s)"):format(ws, jp)
            end
        end
    end)
