...
local function handleAdminDetectedEnhanced(player, reason)
    ...
    local now = tick()
    if now - lastAdminDetect < (Config.DetectionCooldown or 5) then
    ...
    local msg = string.format(
        "🚨 **ADMIN DETECTED!** 🚨\n\n👤 **Player:** %s\n📝 **Display:** %s\n⚠️ **Reason:** %s\n📡 **Status:** %s\n📶 **Ping:** %d ms\n🆔 **PlaceId:** %d\n🌐 **ServerId:** %s\n🔗 **[Join Server](%s)**\n⏱️ **Time:** %s",
        pname, pdisplay, reasonText, status, ping, PlaceId, tostring(JobId), joinLink, timestamp
    )
    ...
    Log("⚠️ Detected admin:", pname, reasonText, "Ping:", ping)

    -- 🔹 Kirim webhook di task.spawn terpisah agar tidak terpotong auto hop
    task.spawn(function()
        local success, err = pcall(function()
        ...
        if not success then
            Log("❌ Failed to send webhook:", err)
        end
    end)
    ...
    if Config.NotifyInGame and type(Notify) == "function" then
        pcall(function()
            Notify("⚠️ Admin Terdeteksi", pname .. " (" .. reasonText .. ") | Ping: " .. ping .. " ms")
        end)
    end
    ...
    -- Delay lebih panjang sebelum auto hop supaya webhook pasti terkirim
    task.spawn(function()
        task.wait(1.0)
...
-- 🔥 Enhanced Summit + Status Notifier (Exit + FC + Ban/Kick + Hop Server + Timestamp + Playtime)
...
local function GetPlaytime()
    local secs = math.floor(tick() - joinTime)
    local mins = math.floor(secs / 60)
    local rems = secs % 60
    return string.format("%dm %ds", mins, rems)
end
...
local function OnSummitReachedEnhanced()
    ...
    if summitStats.count >= SummitAutoHopLimit then
        isHopping = true
        local hopDesc = string.format(
            "🔀 Auto Hop Server aktif!\n👤 Player: %s\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s",
            playerName, ping, GetPlaytime(), GetTimestamp()
        )
        SendWebhookWithReaction("🔀 Hop Server", hopDesc, 0x9b59b6, "🔀")
        AutoHopServer()
    end
end
...
-- === Disconnect Event (Manual Exit vs Hop) ===
-- === Init ===
local oldJobId = tostring(game.JobId)
local lastSeen = tick()
local alreadyOffline = false
local joinTime = tick()

-- === Fungsi kirim webhook notif disconnect/hop/exit ===
local function SendDisconnectNotif(title, color, emoji, extraDesc)
    local playerName = LocalPlayer and LocalPlayer.Name or "Unknown"
    local ping = (LocalPlayer and LocalPlayer.GetNetworkPing and LocalPlayer:GetNetworkPing()) and math.floor(LocalPlayer:GetNetworkPing()*1000) or 0
    local playTime = GetPlaytime()
    local desc = string.format("%s\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s", extraDesc, ping, playTime, GetTimestamp())
    SendWebhookWithReaction(title, desc, color, emoji)
end

-- === Teleport / Hop Server Deteksi (manual + auto hop) ===
if LocalPlayer then
    LocalPlayer.OnTeleport:Connect(function(state)
        if state == Enum.TeleportState.Started then
            alreadyOffline = false
            local newJobId = tostring(game.JobId)
            local joinLink = string.format("roblox://placeId=%d&gameInstanceId=%s", game.PlaceId, newJobId)
            local extraDesc = string.format(
                "🔄 **%s** pindah server!\n🌐 Dari: %s\n➡️ Ke: %s\n🔗 [Join Server Baru](%s)",
                LocalPlayer.Name, oldJobId, newJobId, joinLink
            )
            SendDisconnectNotif("🔄 Player Hop Server", 0x9b59b6, "🔄", extraDesc)
            oldJobId = newJobId
            joinTime = tick()
        end
    end)
...
game:BindToClose(function()
    if alreadyOffline then return end
    alreadyOffline = true
    local extraDesc = string.format("📴 **%s** keluar dari game (Exit Game).", LocalPlayer and LocalPlayer.Name or "Unknown")
    SendDisconnectNotif("📴 Player Keluar Manual", 0xe67e22, "🚪", extraDesc)
end)

-- === Heartbeat (Force Close / Disconnect Unknown) ===
task.spawn(function()
    while task.wait(1) do
        if LocalPlayer and LocalPlayer.Parent == Players then
            ...
            if not alreadyOffline then
                alreadyOffline = true
                local delta = tick() - lastSeen
                if delta <= 5 then
                    local extraDesc = string.format("💥 **%s** keluar tiba-tiba (Force Close).", LocalPlayer and LocalPlayer.Name or "Unknown")
                    SendDisconnectNotif("⚠️ Player Force Close", 0xd35400, "💥", extraDesc)
                else
                    local extraDesc = string.format("❌ **%s** disconnect (Unknown).", LocalPlayer and LocalPlayer.Name or "Unknown")
                    SendDisconnectNotif("📴 Player Offline", 0xe74c3c, "❌", extraDesc)
                end
            end
            break
        end
...
local function AutoHop()
    if alreadyOffline then return end
    alreadyOffline = true
    local extraDesc = string.format("🔀 Auto Hop Server aktif!\n👤 Player: %s", LocalPlayer and LocalPlayer.Name or "Unknown")
    SendDisconnectNotif("🔀 Hop Server", 0x9b59b6, "🔀", extraDesc)
    AutoHopServer()
end
...
task.spawn(function()
    while task.wait(300) do
        ...
        if not (LocalPlayer and LocalPlayer.Parent == Players) then
            alreadyOffline = true
            local extraDesc = string.format("⛔ Akun **%s** dikeluarkan paksa (Ban/Kick).", LocalPlayer and LocalPlayer.Name or "Unknown")
            SendDisconnectNotif("🚫 Player Banned / Kicked", 0xf1c40f, "⛔", extraDesc)
            break
        end
    end
end)
