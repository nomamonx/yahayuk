...
Players.PlayerRemoving:Connect(function(plr)
    if plr == LocalPlayer and not alreadyOffline then
        ...
        local ping = (plr and plr.GetNetworkPing and plr:GetNetworkPing())
            and math.floor(plr:GetNetworkPing() * 1000) or 0
        local oldJobId = tostring(game.JobId)
        local playTime = GetPlaytime()

        task.delay(3, function()
            local newJobId = tostring(game.JobId)
            if LocalPlayer and LocalPlayer.Parent == Players and newJobId ~= oldJobId then
                -- ✅ Hop Server
                local joinLink = string.format("roblox://placeId=%d&gameInstanceId=%s", game.PlaceId, newJobId)
                local desc = string.format(
                    "🔄 **%s** pindah server!\n📶 Ping: %d ms\n⏳ Lama main di server lama: %s\n🌐 Dari: %s\n➡️ Ke: %s\n🔗 [Join Server Baru](%s)\n%s",
                    playerName, ping, playTime, oldJobId, newJobId, joinLink, GetTimestamp()
                )
                SendWebhookWithReaction("🔄 Player Hop Server", desc, 0x9b59b6, "🔄")
                alreadyOffline = false
                joinTime = tick()
            else
                -- ❌ Exit Manual
                local desc = string.format(
                    "📴 **%s** keluar dari game (Exit Game).\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s",
                    playerName, ping, playTime, GetTimestamp()
                )
            ...
            if not alreadyOffline then
                ...
                task.delay(3, function()
                    ...
                    if LocalPlayer and LocalPlayer.Parent == Players and newJobId ~= oldJobId then
                        ...
                        alreadyOffline = false
                        joinTime = tick()
                    elseif tick() - lastSeen <= 2 then
                        -- ⚡ Force Close
                        local desc = string.format(
                            "💥 **%s** keluar tiba-tiba (Force Close).\n📶 Ping: %d ms\n⏳ Lama Main: %s\n%s",
                            playerName, ping, playTime, GetTimestamp()
                        )
