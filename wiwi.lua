...
-- Variabel untuk tracking durasi CP5
local CP5StartTime = tick()
local CP5MinDuration = 60 -- default 60 detik, bisa diubah via slider

-- Reset durasi tiap respawn / relog
local player = game.Players.LocalPlayer
        ...
        if Value then
            autoTeleportTask = task.spawn(function()
                ...
                while isAutoTeleporting do
                    ...
                    -- CEK DURASI DI CP5 sebelum lanjut Puncak
                    local char = player.Character
                    if char and char:FindFirstChild("HumanoidRootPart") then
                        local elapsed = tick() - CP5StartTime
                        if elapsed < CP5MinDuration then
                            char.HumanoidRootPart.CFrame = teleportPoints.CP1
                            Rayfield:Notify({
                                Title = "CP5 Durasi Kurang",
                                Content = "Durasi belum " .. CP5MinDuration .. " detik, balik ke CP1!",
                                Duration = 3,
                            })
                            task.wait(1)
                            continue -- ulang loop dari CP1
                        else
                            Rayfield:Notify({
                                Title = "CP5 Selesai",
                                Content = "Durasi cukup, lanjut ke Puncak!",
                                Duration = 2,
                            })
                        end
                    end
                    ...
                    if char2 and char2:FindFirstChild("HumanoidRootPart") then
                        char2.HumanoidRootPart.CFrame = teleportPoints.Puncak
                        Rayfield:Notify({
                            Title = "Auto Teleport",
                    ...
                    repeat
                        ...
                        if char3 and char3:FindFirstChild("HumanoidRootPart") then
                            if isAtSpawn(char3.HumanoidRootPart.Position, 10) then
                                break
                            end
                        end
...
TeleTab:CreateSection("Atur Waktu Summit")
TeleTab:CreateSlider({
    Name = "⏱ Durasi Minimal CP5 (detik)",
    Range = {5, 300},
    Increment = 1,
    Suffix = "detik",
    CurrentValue = CP5MinDuration,
    Flag = "CP5Duration",
    Callback = function(Value)
        CP5MinDuration = Value
    end,
})

TeleTab:CreateSection("Atur Delay Respawn")
TeleTab:CreateSlider({
